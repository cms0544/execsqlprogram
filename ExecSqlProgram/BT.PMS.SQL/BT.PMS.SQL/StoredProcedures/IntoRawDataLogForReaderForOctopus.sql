IF exists(SELECT * FROM sysobjects WHERE id=object_id(N'IntoRawDataLogForReaderForOctopus') and xtype='P')  DROP PROCEDURE [dbo].[IntoRawDataLogForReaderForOctopus];

/****** Object:  StoredProcedure [dbo].[IntoLog_Octopus]    Script Date: 7/5/2021 9:23:50 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	<插入八達通打卡記錄> 
--EXEC IntoLog_Octopus '000001000000',24,'2216214793','2021-03-30 10:31:31','1','1','true','1','101',11,1,0,'2021-03-30 10:20'
-- =============================================
CREATE PROCEDURE [dbo].[IntoRawDataLogForReaderForOctopus] 
(
	...
	@OldCardNO nvarchar(16)='',--八達通卡替換中的舊卡號,如果是換卡後，新卡會帶有舊卡號
	@IsHistoryRecord bit=1,--記錄八達通是否是即時的（要通過數據庫來判斷是否有權限），還是控制器裏的歷史記錄
	@HistoryRecordValidByOldCardNO bit=0,--歷史拍卡是有效卡，是通過新號還是舊號通過的，1表示是通過舊號
	@EmployeeUserAddress int=0--用戶的卡機位置
)
AS
BEGIN
	SET NOCOUNT ON;
	...
	Declare @UserID int,@UserCode nvarchar(64),@UserName nvarchar(125),@UserType int,@IsOpenDoor int,@OpenDoorTime datetime,@tmpCardID nvarchar(125),@tmpUserAddress int,@tempCount int,@CardType int,@IfHadFace int--samlau 20210304
	--Declare @GetDataTime datetime
	--set @GetDataTime=GetDate()
	Declare @DeviceID int,@Readername nvarchar(125),@AreaID int,@BuildingID int,@IsCardMachine int,@HasQRCode bit,@IsOctDevice bit,@DoorID int,@InOutType int,@IsFirst int--samlau 20210304
	set @DeviceID=0
	set @Readername='' 
	set @AreaID=0
	set @BuildingID=0
	...
	select @DeviceID=HostDeviceID,@Readername=HostName,@AreaID=xq_id,@BuildingID=ly_id,@IsCardMachine=IsCardMachine,@HasQRCode=HasQRCode,@IsOctDevice=IsOctDevice,@DoorID=DoorID,@InOutType=InOutType from V_HostDevice where ReaderLOGO=@ReaderLOGO--samlau 20210304 
    if @DeviceID=0
        begin
	...
	if @IsCardMachine=1 --else if @IsCardMachine=1 and @EventType=2 and @Valid=0
		begin
	...
	VALUES(@DeviceID,@Readername,N'' + @EventTime + '',@EventType,@EventTypeMessage,@CardNO,'',@UserName,@GetDataTime,'',@UserID,@UserCode,@UserName,'','',@InOutType,'',@Valid,@isTAData,@AreaID,@BuildingID,@UserAddress,@UserType,@DoorID,@InOutType,@IsQRCode,0,0,@IsFirst,0,NULL,@IsOpenDoor,@OpenDoorTime,1)--samlau 20210304
